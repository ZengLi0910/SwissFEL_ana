import json
import pathlib
from escape.parse import utilities
import h5py
from dask import array as da
from escape import Array, Scan
import numpy as np
...
def parseSingleFile_v01(file_name,memlimit_0D_MB=5, memlimit_mD_MB=10, createEscArrays=True):
    """Data parser assuming eco-written files from pilot phase 1"""
    lastpath = None
    searchpaths = None
    ...
    if createEscArrays:
        ...
        for name,dat in dstores.items():
            containers[name] = LazyContainer(dat)
            escArrays[name] = Array(containers[name].get_data,\
                    eventIds=containers[name].get_eventIds,stepLengths=dat['stepLengths'],scan=None)
        return escArrays
    else:
        return dstores
